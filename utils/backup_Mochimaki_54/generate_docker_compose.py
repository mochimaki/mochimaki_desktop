...
class DockerComposeGenerator:
        ...
        for app_name, app_info in apps.items():
            ...
            if "data_roots" in app_info:
                for host_path in app_info["data_roots"]:  # リストとして処理
                    container_path = normalize_path(os.path.join(app_path, Path(host_path).name))
                    commands.extend([
    ...
    def _generate_venv_setup_commands(self, user: str, apps: Dict[str, Any]) -> str:
        ...
        for app_name, app_info in apps.items():
            ...
            app_path = f"/home/{user}/apps/{app_name}/{Path(app_info['main']).stem}"
    ...
    def save(self, output_path: str = None):
        ...
        if output_path is None:
            ...
            output_path = os.path.join(os.path.dirname(self.project_info_path), 'docker-compose.yml')
